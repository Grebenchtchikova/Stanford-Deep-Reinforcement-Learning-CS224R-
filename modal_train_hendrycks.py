...
# ---------------------------------------------------------------------------
# Volume commit safety: three layers of protection
#   1. Background thread commits every COMMIT_INTERVAL_S seconds
#   2. SIGTERM handler commits before hard kill arrives
#   3. finally block commits on normal exit or Python-level exceptions
# ---------------------------------------------------------------------------
COMMIT_INTERVAL_S = 300  # 5 minutes
 
 
def _start_periodic_commit(volume, stop_event):
    """Background thread: commit volume every COMMIT_INTERVAL_S seconds."""
    import time
    import threading
...
def _install_sigterm_handler(volume):
    """Catch SIGTERM (Modal sends this before SIGKILL on timeout) and commit."""
    import signal
    import sys
...
def run_train(track: str, total_steps: int, save_freq: int, test_freq: int):
    ...
    print(f"[modal_train] Periodic commit every {COMMIT_INTERVAL_S}s")
 
    # Layer 2: SIGTERM handler
    _install_sigterm_handler(vol)
 
    # Layer 1: background periodic commit
    stop_event = threading.Event()
    commit_thread = _start_periodic_commit(vol, stop_event)
 
    try:
        subprocess.run(cmd, check=True, env=env, cwd="/root/verl")
    finally:
        # Layer 3: finally block (catches Python exceptions + normal exit)
        stop_event.set()
        vol.commit()
        print("[modal_train] Final volume commit done.")
